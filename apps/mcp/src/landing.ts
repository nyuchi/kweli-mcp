// Minimal landing page served at the mount root. The MCP surface itself is the
// mount point exactly (`https://kweli.mukoko.com/mcp`), behind the WorkOS gate;
// this is just a human-facing marker.
export function landingHtml(endpointPath = "/mcp"): string {
  return `<!doctype html>
<html lang="en">
  <head>
    <meta charset="utf-8" />
  ...
  <body>
    ...
    <p>Mukoko Kweli's agent-facing MCP — the trust graph (places, organizations,
      ...
      directly.</p>
    <p>The Model Context Protocol endpoint is at <code>${endpointPath}</code>, gated by
      WorkOS AuthKit — sign-in required, no organization restriction.</p>
